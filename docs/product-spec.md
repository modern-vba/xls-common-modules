...
DIST accepts one independent Distribution Search Root and validates that same invocation-start working directory's ordinal-exact direct `common_modules_repo` as a closed flat package before target mutation. It fixes its candidate set from immediate child project directories only, admits only an ordinal-exact ordinary `common_modules_repo` opt-in directory, and excludes the central source lexically. Exact filename, `LastWriteTimeUtc`, and `Length` metadata alone determine `UNCHANGED`; every other eligible target is fully cleared and receives the recorded package sequentially. Target-local failures do not prevent later candidates from being attempted, while source or Search Root failures stop the run. Distribution does not infer or create opt-ins, follow unsafe reparses, read file contents for identity, stage changes, transact, roll back, retry, or verify the package after copying.
