...
This document is the library specification for the common modules under `xls-common-modules\CommonModules\src\CommonModules`.

Detailed API reference:

- Entry point: `xls-common-modules\CommonModules\doc\html\index.html`
- Member lists by class and standard module: `xls-common-modules\CommonModules\doc\html\annotated.html`, `namespace_class.html`, `namespace_standard.html`
...
`CommonModules/src/CommonModules/common-modules-manifest.tsv` is the canonical machine-readable contract for CommonModules package membership, classifications, direct CommonModule dependencies, and direct external VBA reference requirements. COLLECT locates that manifest through the unique authoring document source set discovered below its explicit Search Root, preserves its validated bytes, and selects each listed source unit by filesystem timestamp and length rather than content or hash. It materializes the manifest, selected source units, and matching form sidecars as a closed flat package in `common_modules_repo` directly below the invocation-start working directory; that package is generated output and never an authoring source. Downstream tooling reads the distributed manifest rather than scraping this Markdown section or VBA source.

DIST accepts one independent Distribution Search Root and validates that same invocation-start working directory's ordinal-exact direct `common_modules_repo` as a closed flat package before target mutation. Package validation reads each source unit only to require strict Windows-932 bytes, an authoritative exact-case `VB_Name` that is non-reserved, no longer than 31 Unicode code points, and matches `^\p{L}[\p{L}\p{Nd}_]*$`, plus extension/source-kind agreement; the manifest remains the sole dependency and reference authority. DIST fixes its candidate set from immediate child project directories only, admits only an ordinal-exact ordinary `common_modules_repo` opt-in directory, and excludes the central source lexically. Exact filename, `LastWriteTimeUtc`, and `Length` metadata alone determine package identity and `UNCHANGED`; every other eligible target is fully cleared and receives the recorded package sequentially. Target-local failures do not prevent later candidates from being attempted, while source or Search Root failures stop the run. Distribution does not infer or create opt-ins, follow unsafe reparses, hash or compare source contents for package identity, stage changes, transact, roll back, retry, or verify the package after copying.
